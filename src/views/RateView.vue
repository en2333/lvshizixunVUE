<template>
  <div style="padding-top: 20px">
    ...
    <el-table
      ...
      <el-table-column prop="lawyerId" label="律师ID" width="100" show-overflow-tooltip/>
      <el-table-column prop="rate" label="评分" width="100" show-overflow-tooltip/>
      <el-table-column prop="text" label="评价内容"  width="300"  show-overflow-tooltip/>
      <el-table-column prop="createTime" label="创建时间"  width="360"  show-overflow-tooltip/>
    ...
    <el-dialog v-model="dialogFormVisible" title="评分">
      <el-form :model="formData">
        <el-form-item label="用户ID" style="float: left" :label-width="formLabelWidth">
          <el-input v-model="formData.userId" style="width: 190px" autocomplete="off" />
        </el-form-item>

        <el-form-item label="律师ID"  :label-width="formLabelWidth">
          <el-input v-model="formData.lawyerId" style="width: 190px" autocomplete="off" />
        </el-form-item>
        ...
        <el-form-item label="评价内容" :label-width="formLabelWidth">
          <textarea placeholder="请输入咨询内容" v-model="formData.text" style="width: 96%;height: 140px"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
const delUser=()=>{
  for (let i = 0; i < selectUser.value.length; i++) {
    $.re.get($.gl.serverPath+"/appraise/bgDelAppraise",{params:{
...
const updateUser=()=>{
  $.re.post($.gl.serverPath+"/appraise/bgUpdAppraise",{
    ...formData.value
  }).then(()=>{
    loadUser()
  })
...
const submitNewUser=()=>{
  $.re.post($.gl.serverPath+"/appraise/bgNewAppraise",{
    ...formData.value
  }).then(()=>{
    loadUser()
  })
...
const loadUser=()=>{
  $.re.get($.gl.serverPath+"/appraise/bgSelAppraise",{params:{
      search: search.value,
